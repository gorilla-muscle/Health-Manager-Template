...
- **Issue の体調チェック(身体の痛み/食欲/眠気・だるさ/その他の4項目・チェック=問題あり)と自分自身への言葉を読み取る**。「身体に痛みがある」チェックまたは痛み・傷病の記載がある場合は傷病検知として扱い、専門家(医師・管理栄養士・トレーナー)への相談を促す(絶対規則6。記録・レポートは継続してよいが医療・診断は行わない)
...
- **体調チェック**: 身体の痛み/食欲/眠気・だるさ/その他の4項目を表示(問題ありは注意色で強調、問題なしは控えめ表示)。痛みがある場合は専門家への相談案内を併記
- **自分自身への言葉**: ユーザー記入の原文をそのまま引用表示する(改変・要約禁止。空欄時は「今日は記入なし」)
- **総評(トレーナーからのまとめ)**: 今日のレポート全体からユーザーへ再度伝えたい要点を、トレーナーがアドバイスするトーンでまとめる(数値根拠1つ以上+明日の最優先アクション1点。専門用語は避ける)
